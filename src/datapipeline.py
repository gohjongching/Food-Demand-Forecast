...
def run_datapipeline():
    ...
    """
    df1 = pd.read_csv(
        "../data/train_GzS76OK/train.csv"
    )  #'./data/train_GzS76OK/train.csv'
    df2 = pd.read_csv(
        "../data/train_GzS76OK/fulfilment_center_info.csv"
    )  #'./data/train_GzS76OK/fulfilment_center_info.csv'
    df3 = pd.read_csv(
        "../data/train_GzS76OK/meal_info.csv"
    )  # './data/train_GzS76OK/meal_info.csv'
    ...
    df_train.drop(
        ["id", "week", "checkout_price", "cuisine", "category"], axis=1, inplace=True
    )
    df_test.drop(
        ["id", "week", "checkout_price", "cuisine", "category"], axis=1, inplace=True
    )
    ...
    df_encoded_df_train = pd.DataFrame(
        df_encoded_train,
        columns=ohe_transformer.get_feature_names_out(one_hot_list),
        index=df_train.index,
    )

    df_encoded_test = ohe_transformer.fit_transform(df_test[one_hot_list]).toarray()
    df_encoded_df_test = pd.DataFrame(
        df_encoded_test,
        columns=ohe_transformer.get_feature_names_out(one_hot_list),
        index=df_test.index,
    )
