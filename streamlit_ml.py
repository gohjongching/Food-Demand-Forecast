...
# in 'Home' page, display the data (EDA)
if app_mode == "Home":
    st.title("PRICE PREDICTION :")
    st.image("streamlit_files/DSC_0422.JPG")
    st.markdown("Dataset :")
    data = pd.read_csv("data/test_QoiMO9B.csv")
    st.write(data.head())
    st.markdown("week VS checkout price ")
    st.bar_chart(data[["week", "checkout_price"]].head(20))
